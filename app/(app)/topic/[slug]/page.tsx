...
export default async function TopicPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const readingText = [topic.summary, ...topic.points, ...topic.units.map((u) => u.unit_name), ...topic.university.map((m) => `${m.module} ${m.institution}`), ...topic.sources.map((x) => `${x.title} ${x.description}`)].join(" ");
  const words = readingText.split(/\s+/).filter(Boolean).length;

  return (
    <article className="mx-auto max-w-4xl space-y-6">
      <header className="card">
        <div className="flex flex-wrap items-center gap-2 text-xs font-semibold text-mute">
          <span className="eyebrow bg-accent/10 text-accent">{topic.curriculum_ref}</span>
        ...
        <h1 className="mt-3 text-3xl font-bold leading-tight tracking-tight text-navy">{topic.title}</h1>
        <p className="mt-2 text-[15px] leading-relaxed text-mute">{topic.summary}</p>
        <div className="mt-3 flex flex-wrap items-center gap-2">
          ...
          {lock.locked ? <span className="badge-warn">Terkunci</span> : <span className={st.done ? "badge-ok" : "badge-warn"}>{st.done ? "Topik selesai" : "Belum selesai: perlu kuis lulus"}</span>}
        </div>
      </header>
        ...
        <div className="mt-3 grid gap-4 md:grid-cols-2">
          <div>
            ...
            <ul className="space-y-2 text-sm">
              {topic.units.map((u) => (
                <li key={u.unit_code}>
                  <a href={u.url} target="_blank" rel="noreferrer" className="font-mono text-xs text-accent-2 hover:underline">{u.unit_code}</a>
                  <span className="ml-1">{u.unit_name}</span>
                  {u.is_core && <span className="badge-accent ml-1">inti</span>}
                </li>
              ))}
            </ul>
          </div>
      ...
      <section className="card" id="rangkuman">
        ...
        {st.quiz_passed ? (
          <>
            <p className="mt-1 text-sm text-mute">Kuis sudah lulus. Rangkuman ini menandai kamu sudah menuntaskan materi topik ini.</p>
